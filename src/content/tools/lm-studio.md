...
slug: lm-studio
title: LM Studio
tagline: Free desktop app for running open-weight LLMs locally. Visual model browser, one-click download, OpenAI- and Anthropic-compatible local server, and LM Link for remote instances. Best GUI alternative to Ollama for non-CLI users.
...
facts:
  ...
  pricing_anchor:
    value: "LM Studio is now free for both home and work use, so commercial-license friction is gone; evaluate model licenses and hardware costs separately."
    source: https://lmstudio.ai/
    source_label: LM Studio official site
    source_id: lm-studio-official
    verified_at: '2026-05-13'
  ...
  local_runtime:
    ...
    next_review_at: '2026-11-13'
  api_available:
    value: "LM Studio exposes a local API with OpenAI-compatible and Anthropic Messages-compatible endpoints, MCP support, and LM Link for connecting to remote LM Studio instances."
...
seo_title: "LM Studio: Features, Pricing & Review (May 2026)"
meta_description: "LM Studio is the free desktop GUI for running open-weight LLMs locally. Visual model browser, one-click download, OpenAI- and Anthropic-compatible local server, and LM Link for remote instances. Free for home and work use."
...
quick_answer: >-
  LM Studio is the free desktop app that makes running open-weight LLMs on your laptop a 5-minute setup. Visual model browser, one-click quantization, and a built-in local server with OpenAI- and Anthropic-compatible endpoints plus LM Link for remote instances. Free for both home and work use as of 2026. Pick it for a GUI alternative to Ollama; skip it for server workloads or pure CLI tooling.
---
...
> **Pick LM Studio if you want the easiest path to local LLMs on a desktop.** The visual model browser is genuinely helpful when you're choosing between quantizations. Chat interface, model downloads, and a local server speaking both OpenAI and Anthropic Messages dialects all ship in one application. Mac, Windows, Linux builds. LM Link lets you connect a thin client to a beefier machine across the network.
>
...
>
> **Free for home and work use, period.** LM Studio dropped the separate commercial-license requirement in 2026. No tier system, no features behind a paywall, no sales contact needed for business desktops. That's the whole pricing model.

## Key Facts

| | |
|---|---|
| **Current version** | 0.4.12 (May 2026) |
...
| **Local server** | OpenAI-compatible HTTP endpoints, Anthropic Messages API compatibility, MCP client support |
| **LM Link** | Connect to remote LM Studio instances and load their models as if local |
| **SDKs** | `lmstudio-js` (TypeScript) and `lmstudio-python` |
...
Verified 2026-05-13 via [lmstudio.ai](https://lmstudio.ai). The terms shifted in 2026 to drop the prior "contact us for commercial" gating; review the [LM Studio Terms of Use](https://lmstudio.ai/terms) before deploying at scale.
...
- **Read the terms before mass deployment.** LM Studio dropped its commercial-license gate in 2026, but redistribution, embedding LM Studio inside a sold product, or large fleet rollouts can still hit specific clauses worth confirming.
...
Produced by the aipedia.wiki editorial pipeline. Last verified 2026-05-13 against [lmstudio.ai](https://lmstudio.ai), the [LM Studio local API docs](https://lmstudio.ai/docs/app/api), and [aiagentslist.com 2026 LM Studio review](https://aiagentslist.com/agents/lm-studio).

## FAQ

**Is LM Studio really free?**
Yes. As of 2026 the site states "LM Studio is free for home and work use," so the separate commercial-license conversation is no longer required for ordinary business desktops. Redistribution and embedding LM Studio inside a sold product are still worth checking against the [Terms of Use](https://lmstudio.ai/terms).
...
Same underlying inference (both use llama.cpp derivatives). LM Studio is GUI-first and desktop-focused, and its 2026 builds expose Anthropic-compatible endpoints and the LM Link remote-instance feature alongside the OpenAI-compatible server. [Ollama](/tools/ollama/) stays CLI-first with a lightweight HTTP server, which is still better for scripting and server deployments.
