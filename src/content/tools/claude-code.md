...
Core pricing and model-access data above was verified against vendor sources and current reporting on 2026-06-14. See Sources.
...
Claude Opus 4.8 is the current stable high-end Anthropic model for hard Claude Code work as of June 15, 2026. Fable 5 and Mythos 5 are suspended, so treat them as unavailable until Anthropic restores access. Sonnet 4.6 and Haiku 4.5 are selectable for cost or speed trade-offs. The 1M token context window applies to Opus and Sonnet.

**Do Agent SDK credits cover normal Claude Code sessions?**
No. Anthropic says the monthly credits cover Claude Agent SDK, `claude -p`, Claude Code GitHub Actions, and third-party Agent SDK apps. Interactive Claude Code in the terminal or IDE still uses normal subscription usage limits.
...
- [Claude Agent SDK credit help](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan): June 15 Agent SDK, `claude -p`, and GitHub Actions credit rules
