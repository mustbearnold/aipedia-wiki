...
- `npm run agent:proof:readiness -- --json`: check whether bounded proof targets are safe to run or already proved. Page-refresh and tool-refresh targets fail closed on stale generated inputs or dirty content boundaries; affiliate handoff reports `proved` once its durable proof receipt validates.
...
After recording a meta-goal closeout, validate the receipt contract with `npm run agent:meta:closeout:auto -- --json` for the latest loop receipt, or add `--receipt <path>` for an explicit proof receipt. The router chooses the strict latest-loop, explicit-loop, or Rust-runner profile and fails closed on unsupported receipt types. `agent:meta:closeout`, `agent:meta:closeout:receipt`, and `agent:meta:closeout:runner` remain the exact underlying commands when a workflow needs a named validator. Use `npm run agent:closeout:check -- --all-system --json` for a backwards-compatible health check across historical system receipts.
