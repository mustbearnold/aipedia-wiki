...
- `npm run agent:proof:readiness -- --json`: check whether bounded proof targets are safe to run. Page-refresh and tool-refresh targets fail closed on stale generated inputs or dirty content boundaries; affiliate handoff can be ready when its inventory input is fresh.
