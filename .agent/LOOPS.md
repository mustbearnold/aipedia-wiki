...
- `npm run agent:proof:readiness -- --json`: check whether bounded proof targets are safe to run or already proved. Page-refresh and tool-refresh targets fail closed on stale generated inputs or dirty content boundaries; affiliate handoff reports `proved` once its durable proof receipt validates. Add `--out <path>` when the blocked or proved state should become a durable receipt.
