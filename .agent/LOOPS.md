...
- `npm run agent:meta:closeout:auto -- --receipt <path> --json`: route a latest loop, explicit loop, Rust runner, or durable efficiency-trend receipt to the correct June 30 closeout profile. Use this as the default when the receipt type is the only thing known.
...
After recording a meta-goal closeout, validate the receipt contract with `npm run agent:meta:closeout:auto -- --json` for the latest loop receipt, or add `--receipt <path>` for an explicit proof receipt. The router chooses the strict latest-loop, explicit-loop, Rust-runner, or efficiency-trend profile and fails closed on unsupported receipt types. `agent:meta:closeout`, `agent:meta:closeout:receipt`, and `agent:meta:closeout:runner` remain the exact underlying commands when a workflow needs a named validator. Use `npm run agent:closeout:check -- --all-system --json` for a backwards-compatible health check across historical system receipts.
