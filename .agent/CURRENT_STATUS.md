...
The first new approved-tool page after the metadata foundation is locally verified and subagent-accepted but not yet pushed: `/guides/dext-pricing-for-bookkeeping-firms/`. It targets Dext pricing for bookkeeping firms, links from AI Automation, and keeps the broader receipt-tool guide as a sibling rather than a duplicate cluster. Visual/mobile review accepted at 9.9/10 after the related-link polish was fixed. Accuracy/SEO review accepted at 9.9/10 after Hubdoc was narrowed to Xero-included or standalone positioning. Current focused gates passed: strict affiliate audit, frontmatter, em dash guard, ledger check, typecheck, source health, build:fast, commercial CTA audit, and route QA across the new guide, receipt sibling, AI Automation, `/guides/`, Dext tool page, and accountant stack at 319, 360, 390, 430, 768, 1024, and 1366 px.

Next affiliate move after pushing the Dext slice: keep building Dext to a distinct cluster of up to five pages only where intent is unique. Candidate next angles are a same-job switcher from Hubdoc for multi-client firms, an accountant workflow stack page refresh that makes Dext the document-intake anchor, or a Dext vs AutoEntry same-job comparison for Sage-heavy bookkeeping workflows. Do not create a page unless the buyer job is distinct and source-supported.
