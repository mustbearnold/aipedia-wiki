...
- Latest system slice: extended Rust runner tool-refresh and page-refresh closeout JSON with optional `system_progress` from `agent-system-progress-check.mjs`, and tightened `agent:closeout:check` so runner receipts validate that field when present.
...
- The system goal is not complete. Next system work should add goal_id/run_id and risk fields to closeout receipts, extend stale-input gates beyond coverage backlog, improve non-stale scoring and score decay, and then prove the changes on one bounded real workload with receipts.
...
1. Add goal_id/run_id and residual-risk fields to closeout receipts.
2. Extend stale-input policy to other planners and queues that can drive stale work.
3. Add non-stale scoring fields, risk/confidence decay, calibration checks, and tests.
