...
- Decision loop verification is now more exact for rendered content because changed comparison, tool, category, and parent-hub routes run through combined route QA. Broad `smoke:visual` is now skipped for content-only route cycles. The remaining cleanup target is to move this allowlist into `src/data/operator-surfaces.json` or an equivalent contract-level model so future agents see the rule in one place.
