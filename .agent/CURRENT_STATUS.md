...
- Branch: `master`. Active goal is the affiliate conversion page buildout across tools with configured affiliate links. The June 27 strict 3-day whole-site refresh target is complete. Latest affiliate slice is the accepted Dext vs AutoEntry Sage-bookkeeper guide plus a shared mobile sticky CTA containment fix.
...
Current affiliate move: `/guides/dext-vs-autoentry-for-sage-bookkeepers/` is verified and subagent-accepted as the Sage-heavy same-job switcher for bookkeepers deciding between Dext's practice workflow and AutoEntry's Sage-native credit model. Focused gates passed: strict affiliate audit, frontmatter, guide-picks audit, em dash guard, ledger check, source health, typecheck, build:fast, built commercial CTA audit, route QA across the new guide plus affected sibling, hub, tool, and workflow routes at 319, 360, 390, 430, 768, 1024, and 1366 px, plus targeted Playwright sticky CTA inspection at 319, 360, and 390 px. Accuracy review accepted at 9.9/10. Visual/mobile review rejected the first pass for a shared sticky CTA scoped-style bug, then accepted at 9.9/10 after `StickyCTA.astro` was hardened.
