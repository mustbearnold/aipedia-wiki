...
Latest affiliate move: the Argil cluster is locally verified with `/guides/argil-pricing-for-ugc-avatar-video-teams/`, a plan-decision page for creators, ecommerce marketers, agencies, and founders choosing Classic, Pro, Scale, or Enterprise without misreading credit burn. It updates the Argil tool page, the avatar-video guide, Synthesia alternatives, AI Video, `/guides/`, `/tools/`, `/categories/`, `/`, `/explore/`, `/search/`, `/llms.txt`, `/llms-full.txt`, source registry rows, and `PAGE_REFRESH_LEDGER.md`. The pass also adds three closeout guardrails: `audit:date-consistency`, structured runner closeout JSON receipts with superseded-failure tracking, and global URL dedupe/cache metrics in `page:source-health`. Page source health caught an obsolete MiniMax video API URL and two Adobe HelpX timeouts on AI Video; the MiniMax URL was replaced with the current guide route, and the fragile Adobe HelpX citations were removed from that parent page after the visible Adobe claim was narrowed to sources that pass the checker. Final focused gates passed: source health, changed frontmatter, strict affiliate conversion, changed provenance, fact audit, ledger, guard, live selected sources, command audit, links/news/security, script tests, Rust runner tests, typecheck, `build:fast`, search/LLM surface checks, metadata/disclosure checks, QA agent metrics, and route QA across affected routes at 319, 360, 390, 430, 768, 1024, and 1366 px.
...
Latest Argil affiliate conversion slice passed:
...
- `npm run --silent audit:sources -- --json --live --limit 0 ...` across 15 Argil, competitor, MiniMax, and Adobe source IDs
- `npm run test:scripts`
- `cargo test --manifest-path tools/aipedia-runner/Cargo.toml`
- `npm run typecheck`
- `AIPEDIA_CURRENT_DATE=2026-06-28 npm run build:fast`
- `npm run qa:route -- --site-dir dist-fast/client --concurrency 6 --widths 319,360,390,430,768,1024,1366 ...`
...
- Generated surface checks confirmed the new guide in home search, `/guides/`, `/llms.txt`, and `/llms-full.txt`; rendered metadata checks confirmed canonical, meta description, affiliate disclosure, and Argil guide links on the new guide, Argil tool page, and AI Video category.
