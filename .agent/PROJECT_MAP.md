...
- Multi-loop registry is active. Use `npm run loop:system` to list loops, `npm --silent run loop:all -- --json` to run a machine-readable loop review, and `npm --silent run loop:all:record -- --json` to persist a broad-review receipt.
...
- Run `npm run loop:system` to list registered operating loops and `npm --silent run loop:all -- --json` to review all loop signals. Recorded broad reviews live under `.agent/loop-runs/system/`.
