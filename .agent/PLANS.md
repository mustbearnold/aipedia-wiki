...
- Slices 01 through 87 are active. Slice 87 adds closeout-checkable post-default routing monitor and rollback receipts; the next system target after Slice 87 closeout is broader runtime integration so deployed routing changes must attach monitor receipts automatically. Pre-existing Captions/Synthesia content WIP remains separate and must not be staged with system commits.
