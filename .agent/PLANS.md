...
- Slices 01 through 05 are verified locally. Slice 01 added stale backlog enforcement, system-progress checking, durable pause receipts, compliance docs, and focused tests. Slice 02 wired system-progress into `loop:all:record` receipts and added `--require-system-progress` for meta-goal closeouts. Slice 03 added `agent:closeout:check` receipt validation. Slice 04 extended Rust runner closeout receipts with `system_progress`. Slice 05 standardized `goal_id`, `run_id`, `residual_risks`, and `next_actions` across loop and Rust runner closeouts. Pre-existing Captions/Synthesia content WIP remains separate and must not be staged with system commits.
...
1. Extend stale-input gates to page/tool/affiliate planners that can drive stale work.
2. Add non-stale scoring with risk/confidence fields, score decay, calibration checks, and tests.
