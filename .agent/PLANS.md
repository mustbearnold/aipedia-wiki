...
- Slices 01 through 92 are active. Slice 92 adds closeout-checkable longer-window routing monitor trend rollups. The next system target is to collect a second closeout-checked monitor-trend receipt after the next runtime or workflow change, then regenerate the rollup to seek `rollup-ready`. Pre-existing Captions/Synthesia content WIP remains separate and must not be staged with system commits.
