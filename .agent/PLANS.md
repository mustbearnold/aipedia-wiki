...
1. Add goal_id, run_id, residual-risk, and next-action fields to closeout receipts.
2. Extend stale-input gates to page/tool/affiliate planners that can drive stale work.
3. Add non-stale scoring with risk/confidence fields, score decay, calibration checks, and tests.
4. Run one bounded production pilot only after the next system slice is verified, then record receipts and update memory.
...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --json`.
