...
- `npm --silent run agent:proof:readiness -- --json`: read-only readiness check for bounded meta-goal proof pilots. It reports page-refresh, tool-refresh, and affiliate-handoff proof readiness, including stale generated inputs, dirty content boundaries where those affect the proof target, and validated durable proof receipts as `proved`. Add `--out <path>` to write a closeout-checkable `aipedia.meta-proof-readiness.v1` receipt.
