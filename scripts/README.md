...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --json`: validates loop or runner closeout receipt JSON contracts and can require enforced system-progress evidence plus `goal_id`, `run_id`, `residual_risks`, and `next_actions`. Use `--require-workflow-policy` on tool-refresh and page-refresh runner receipts to enforce workflow-specific commands, artifacts, standard widths, `system_progress`, and fresh `input_freshness`.
...
- `npm run agent:closeout:check`: validates loop-run and Rust runner closeout JSON receipts. Use `-- --require-system-progress --require-closeout-identity` for June 30 meta-goal closeouts, and add `--require-workflow-policy` for runner closeout receipts.
...
- `agent-closeout-receipt-check.mjs`: validates system loop and Rust runner closeout receipts, including optional enforced system-progress, closeout-identity, trace-artifact, runner input-freshness, and workflow-policy fields.
