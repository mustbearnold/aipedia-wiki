...
- `npm --silent run agent:efficiency:trends -- --max-runs 10 --json`: aggregates recent system loop receipts into speed and receipt-size trend summaries. Add `--fail-on-missing-metrics` when the selected receipt window must be fully metric-aware.
