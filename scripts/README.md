...
- `npm run agent:closeout:check`: validates loop-run, Rust runner closeout, and affiliate handoff JSON receipts. Use `npm run agent:meta:closeout -- --json` for latest June 30 meta-goal loop closeouts, or `npm run agent:meta:closeout:receipt -- --receipt <path> --json` when the primary proof is an explicit runner or copied receipt. Add `--require-workflow-policy` for runner closeout receipts or affiliate handoff receipts.
