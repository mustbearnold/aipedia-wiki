...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --json`: validates loop, runner closeout, and affiliate handoff receipt JSON contracts and can require enforced system-progress evidence plus `goal_id`, `run_id`, `residual_risks`, and `next_actions`. Use `--require-workflow-policy` on tool-refresh and page-refresh runner receipts to enforce workflow-specific commands, artifacts, standard widths, `system_progress`, and fresh `input_freshness`; use it on affiliate handoff receipts to enforce selected clusters, evidence counts, route QA scope, verification gates, and no-edit boundaries.
