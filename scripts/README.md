...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --require-efficiency-metrics --json`: validates loop, runner closeout, affiliate handoff, and pause receipt JSON contracts and can require enforced system-progress evidence, closeout identity, trace artifacts, loop efficiency metrics, and checked DAG proof artifacts. Use `--require-workflow-policy` on tool-refresh and page-refresh runner receipts to enforce workflow-specific commands, artifacts, standard widths, `system_progress`, and fresh `input_freshness`; use it on affiliate handoff receipts to enforce selected clusters, evidence counts, route QA scope, verification gates, and no-edit boundaries.
...
- `npm --silent run agent:efficiency:trends -- --max-runs 10 --json`: aggregates recent system loop receipts into speed, receipt-size, stability, and correction-rate summaries. Add `--fail-on-missing-metrics` when the selected receipt window must be fully metric-aware, and `--out <path>` when the trend proof should become a closeout-checkable `aipedia.loop-efficiency-trends.v1` receipt.
...
- `npm run loop:all:record`: runs every registered loop and writes a JSON receipt plus `latest.json` under `.agent/loop-runs/system/`, including closeout identity fields, `system_progress`, and `efficiency_metrics` for wall time, command time, artifact counts, attention rates, receipt size, and slowest commands.
