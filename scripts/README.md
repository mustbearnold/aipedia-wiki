...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --require-efficiency-metrics --json`: validates loop, runner closeout, affiliate handoff, and pause receipt JSON contracts and can require enforced system-progress evidence, closeout identity, trace artifacts, and loop efficiency metrics. Use `--require-workflow-policy` on tool-refresh and page-refresh runner receipts to enforce workflow-specific commands, artifacts, standard widths, `system_progress`, and fresh `input_freshness`; use it on affiliate handoff receipts to enforce selected clusters, evidence counts, route QA scope, verification gates, and no-edit boundaries.
...
- `npm run agent:closeout:check`: validates loop-run, Rust runner closeout, and affiliate handoff JSON receipts. Use `-- --require-system-progress --require-closeout-identity` for June 30 meta-goal closeouts, and add `--require-workflow-policy` for runner closeout receipts or affiliate handoff receipts.
...
- `agent-closeout-receipt-check.mjs`: validates system loop, Rust runner closeout, and affiliate handoff receipts, including optional enforced system-progress, closeout-identity, trace-artifact, runner input-freshness, and workflow-policy fields.
