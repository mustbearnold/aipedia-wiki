...
- `npm --silent run agent:meta:closeout:auto -- --receipt <path> --json`: strict June 30 closeout router. Use it as the default handoff command when unsure; it validates latest loop receipts, explicit loop receipts, Rust runner receipts, and durable loop-efficiency trend receipts with the correct profile, and fails closed on unsupported receipt types.
...
- `npm run agent:closeout:check`: validates loop-run, Rust runner closeout, affiliate handoff, proof-readiness, loop-efficiency trend, runner-interrupt proof, and pause JSON receipts. Use `npm run agent:meta:closeout:auto -- --json` for default June 30 meta-goal closeout routing, or pass `--receipt <path>` to let the router choose explicit loop, runner, or efficiency-trend policy. Keep `agent:meta:closeout`, `agent:meta:closeout:receipt`, and `agent:meta:closeout:runner` available when a workflow needs the exact underlying command.
