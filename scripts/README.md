...
- `npm run agent:closeout:check`: validates loop-run, Rust runner closeout, and affiliate handoff JSON receipts. Use `npm run agent:meta:closeout -- --json` for latest June 30 meta-goal loop closeouts, `npm run agent:meta:closeout:receipt -- --receipt <path> --json` for explicit loop or eval receipts, and `npm run agent:meta:closeout:runner -- --receipt <path> --json` for DAG-planned runner receipts that must also satisfy workflow policy.
