...
- `npm run agent:closeout:check`: validates loop-run, Rust runner closeout, and affiliate handoff JSON receipts. Use `npm run agent:meta:closeout -- --json` for June 30 meta-goal closeouts so required system progress, identity, trace artifacts, efficiency metrics, and DAG proof cannot be skipped by forgetting flags. Add `--require-workflow-policy` for runner closeout receipts or affiliate handoff receipts.
