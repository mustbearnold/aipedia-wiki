...
- `npm --silent run agent:pause-receipt -- --goal-id <id> --safe-resume-step <step> --in-progress-step <step> --json`: writes a structured pause/resume receipt with dirty-tree state, validation done/pending, must-not-repeat items, and next commands. Use `--validate <path>` to check an existing pause receipt.
...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --require-efficiency-metrics --json`: validates loop, runner closeout, and affiliate handoff receipt JSON contracts and can require enforced system-progress evidence, closeout identity, and loop efficiency metrics. Use `--require-workflow-policy` on tool-refresh and page-refresh runner receipts to enforce workflow-specific commands, artifacts, standard widths, `system_progress`, and fresh `input_freshness`; use it on affiliate handoff receipts to enforce selected clusters, evidence counts, route QA scope, verification gates, and no-edit boundaries.
...
- `npm run agent:pause-receipt`: writes `.agent/loop-runs/pauses/*-pause-receipt.json` unless `--out` is supplied. Long-running goals should use it before pausing or handing off. It also validates `aipedia.pause-receipt.v1` receipts with `--validate` or `--receipt`.
...
- `agent-pause-receipt.mjs`: writes and validates structured JSON pause receipts so future agents can resume from files rather than chat memory.
