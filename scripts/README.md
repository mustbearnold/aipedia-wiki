...
- `npm --silent run agent:closeout:check -- --receipt .agent/loop-runs/system/latest.json --require-system-progress --require-closeout-identity --json`: validates loop or runner closeout receipt JSON contracts and can require enforced system-progress evidence plus `goal_id`, `run_id`, `residual_risks`, and `next_actions`. Runner receipts are also checked for embedded `input_freshness` shape when present.
