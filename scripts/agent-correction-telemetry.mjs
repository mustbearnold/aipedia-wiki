#!/usr/bin/env node
// Build a deterministic correction telemetry receipt for routing and quality evaluation.

import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildCorrectionTelemetry, correctionIssue } from './lib/correction-telemetry.mjs';
...
if (!existsSync(inputFile)) {
  issues.push(correctionIssue('correction-telemetry-input-missing', `${projectPath(inputFile)} does not exist.`));
} else {
  try {
    input = JSON.parse(readFileSync(inputFile, 'utf8'));
  } catch (error) {
    issues.push(correctionIssue('correction-telemetry-input-invalid', `${projectPath(inputFile)} could not parse as JSON: ${error.message}`));
  }
}
...
const result = buildCorrectionTelemetry(input, {
...
function usage() {
  return [
    'Usage:',
    '  node scripts/agent-correction-telemetry.mjs --input <path> --json',
    '  node scripts/agent-correction-telemetry.mjs --input <path> --out <path> --json',
    '',
    'Builds an aipedia.correction-telemetry.v1 receipt from exact finding, correction, residual, and regression events.',
    'Routing evaluations can consume this receipt with agent:routing:evaluate -- --correction-telemetry <path>.',
    '',
    'Options:',
    '  --input <path>        Input JSON with correction event telemetry.',
