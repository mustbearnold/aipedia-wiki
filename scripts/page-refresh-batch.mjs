...
const FAIL_ON_STALE_LEDGER = args.includes('--fail-on-stale-ledger') || args.includes('--fail-on-stale-inputs');
...
function usage() {
  return [
    ...
    '  --fail-on-stale-ledger  Fail when the default PAGE_REFRESH_LEDGER.md is not current.',
    '  --fail-on-stale-inputs  Alias for --fail-on-stale-ledger.',
...
function ledgerInputFreshness() {
  if (!existsSync(LEDGER_PATH)) {
    return {
      ...
      exit_code: null,
      changed: null,
      ledger_updated_through: '',
      next_action: 'Run npm run ledger:pages, then rerun the page refresh planner.',
    };
  }

  const defaultLedgerPath = resolve(PROJECT_DIR, 'PAGE_REFRESH_LEDGER.md');
  if (resolve(LEDGER_PATH) !== defaultLedgerPath) {
    return {
      ok: true,
      kind: 'page-refresh-ledger',
      status: 'custom-ledger-unchecked',
      ledger: projectPath(LEDGER_PATH),
      command: '',
      exit_code: null,
      changed: null,
      ledger_updated_through: '',
      next_action: 'Custom ledger path cannot be regenerated by the strict default-ledger check.',
    };
  }

  const commandArgs = [
    resolve(PROJECT_DIR, 'scripts/generate-page-refresh-ledger.mjs'),
    '--check',
    '--json',
    '--project-dir',
    PROJECT_DIR,
  ...
  const ok = child.status === 0 && parsed?.ok === true;
  return {
    ok,
    kind: 'page-refresh-ledger',
    ...
    ledger_updated_through: parsed?.ledger_updated_through || '',
    stdout_tail: tail(child.stdout),
    stderr_tail: tail(child.stderr),
    next_action: ok ? '' : 'Run npm run ledger:pages && npm run ledger:pages:check, then rerun the page refresh planner.',
  };
}
...
function staleLedgerReport(inputFreshness) {
  return {
    ok: false,
    mode: 'stale-inputs',
...
function emitStaleLedgerReport(reportData) {
  if (JSON_MODE) {
    process.stdout.write(`${JSON.stringify(reportData, null, 2)}\n`);
    return;
  }
  console.error(`[page-refresh-batch] ${reportData.input_freshness.next_action}`);
}
...
const inputFreshness = ledgerInputFreshness();
if (FAIL_ON_STALE_LEDGER && !inputFreshness.ok) {
  emitStaleLedgerReport(staleLedgerReport(inputFreshness));
  process.exit(1);
}
...
const report = {
  ...
  ledger: projectPath(LEDGER_PATH),
  fail_on_stale_inputs: FAIL_ON_STALE_LEDGER,
  input_freshness: inputFreshness,
