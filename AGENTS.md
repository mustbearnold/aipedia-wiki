...
Astro builds the public website. `tools/project-kg/` understands the website as a local tooling layer for Codex/Hermes. It scans the repository, writes a SQLite graph to `.kg/project.db`, and generates human-readable findings in `reports/project-kg-report.md`.

Website folders:

- `src/content/` is the content source of truth.
- `src/pages/`, `src/layouts/`, `src/components/`, `src/styles/`, `src/data/`, and `src/utils/` power the Astro site.
- `public/logos/`, `public/og/`, `public/icons/`, and `public/images/` are public assets. `public/og/` is generated by scripts and should be reviewed carefully before staging.
- `scripts/` contains site guards, content audits, build helpers, logo generation, OG generation, and deployment helpers.

Knowledge graph folders:

- `tools/project-kg/` contains the TypeScript CLI and graph extraction code.
- `.kg/` stores local runtime graph data. Do not commit `.kg/project.db`, cache, logs, embeddings, or snapshots.
- `.kg/exports/` can hold future shareable graph exports.
- `reports/` stores human-readable graph reports.
- `agent-runs/` stores records of Codex/Hermes tasks.

Safe edit zones:

- Website source edits normally belong in `src/`, `public/`, or `scripts/`.
- Knowledge graph tooling edits belong in `tools/project-kg/`.
- Task notes belong in `agent-runs/`.
- Human review artifacts belong in `reports/`.

Runtime or generated folders that should not be manually edited:

- `.kg/project.db`, `.kg/cache/`, `.kg/logs/`, `.kg/embeddings/`, `.kg/snapshots/`
- `.astro/`, `dist/`, `dist-fast/`, `public/pagefind/`
- Generated assets in `public/og/`, `src/data/_meta/`, and `src/data/logo-manifest.json` unless the task explicitly requires generated output.
...
Run `npm run build:fast` after Astro route/template/data changes. Run `npm run build:full:node22` for final local production confidence when appropriate.

Codex/Hermes graph workflow:

- Remember one entry point: `npm run kg -- <command>`. Run `npm run kg -- help` to list commands.
- Run `npm run kg -- scan` and `npm run kg -- report` after structural or content changes.
- Use `npm run kg -- related <path>` before editing unfamiliar files to see connected pages, imports, scripts, and assets.
- Use `npm run kg -- depends-on <path>` before renaming or moving a file.
- Use `npm run kg -- unused-assets`, `npm run kg -- missing-frontmatter`, and `npm run kg -- broken-links` before cleanup tasks.

Do not touch legacy backup folders, especially `../wikis/ai-tools.legacy-backup-2026-04-17/`, unless the user explicitly asks.
