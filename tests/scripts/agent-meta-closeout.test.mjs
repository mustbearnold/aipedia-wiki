...
test('meta closeout router validates routing policy receipts', () => {
  const root = mkdtempSync(join(tmpdir(), 'aipedia-meta-closeout-routing-policy-'));
  try {
    writeJson(join(root, 'routing-policy.json'), validRoutingPolicyReceipt());

    const result = runRouter(['--project-dir', root, '--receipt', 'routing-policy.json', '--json']);
    assert.equal(result.status, 0, result.stderr || result.stdout);
    const report = JSON.parse(result.stdout);
    assert.equal(report.ok, true);
    assert.equal(report.closeout_profile, 'routing-policy');
    assert.equal(report.requires_workflow_policy, false);
    assert.deepEqual(report.strict_flags, []);
...
test('meta closeout router validates routing monitor receipts', () => {
  const root = mkdtempSync(join(tmpdir(), 'aipedia-meta-closeout-routing-monitor-'));
  try {
    writeJson(join(root, 'routing-monitor.json'), validRoutingMonitorReceipt());

    const result = runRouter(['--project-dir', root, '--receipt', 'routing-monitor.json', '--json']);
    assert.equal(result.status, 0, result.stderr || result.stdout);
    const report = JSON.parse(result.stdout);
    assert.equal(report.ok, true);
    assert.equal(report.closeout_profile, 'routing-monitor');
    assert.equal(report.requires_workflow_policy, false);
    assert.deepEqual(report.strict_flags, []);
...
test('meta closeout router validates routing monitor trend receipts', () => {
  const root = mkdtempSync(join(tmpdir(), 'aipedia-meta-closeout-routing-monitor-trends-'));
  try {
    writeJson(join(root, 'routing-monitor-trends.json'), validRoutingMonitorTrendsReceipt());

    const result = runRouter(['--project-dir', root, '--receipt', 'routing-monitor-trends.json', '--json']);
    assert.equal(result.status, 0, result.stderr || result.stdout);
    const report = JSON.parse(result.stdout);
    assert.equal(report.ok, true);
    assert.equal(report.closeout_profile, 'routing-monitor-trends');
    assert.equal(report.requires_workflow_policy, false);
    assert.deepEqual(report.strict_flags, []);
...
test('meta closeout router validates routing runtime completion receipts', () => {
  const root = mkdtempSync(join(tmpdir(), 'aipedia-meta-closeout-routing-runtime-completion-'));
  try {
    writeJson(join(root, 'routing-runtime-completion.json'), validRoutingRuntimeCompletionReceipt());

    const result = runRouter(['--project-dir', root, '--receipt', 'routing-runtime-completion.json', '--json']);
    assert.equal(result.status, 0, result.stderr || result.stdout);
    const report = JSON.parse(result.stdout);
    assert.equal(report.ok, true);
    assert.equal(report.closeout_profile, 'routing-runtime-completion');
    assert.equal(report.requires_workflow_policy, false);
    assert.deepEqual(report.strict_flags, []);
