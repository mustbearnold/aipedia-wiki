...
- `npm run loop:system`: lists the registered AiPedia operating loops; use `npm --silent run loop:all -- --json` for a machine-readable loop review.
- `npm --silent run loop:all:record -- --json`: runs the broad loop review and writes a machine-readable receipt under `.agent/loop-runs/system/`.
