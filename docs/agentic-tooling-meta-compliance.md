...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, `agent:closeout:check --require-efficiency-metrics`, `agent:efficiency:trends`, `stability_summary`, `correction_summary`, focused loop, closeout, and trend tests, live 3-run correction-rate trend proof | Add exact model token usage and exact correction counts when the runtime exposes them. |
