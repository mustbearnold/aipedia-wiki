...
| System-progress checkpoint | Partial | `agent:system-progress`, `agent:system-progress --observed-dirty-before-agent`, `loop:all:record --require-system-progress`, loop `system_progress.agent_*` and `system_progress.preexisting_*` fields, scoped loop efficiency artifact counts, Rust runner `system_progress` closeout field, runner `AIPEDIA_OBSERVED_DIRTY_BEFORE_AGENT` passthrough | Keep enforcing on every meta closeout and pilot. |
...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, `agent:closeout:check --require-efficiency-metrics`, `agent:efficiency:trends --out`, durable `aipedia.loop-efficiency-trends.v1` receipts, `agent:closeout:check` loop-efficiency-trends validation, `agent:meta:closeout:auto` efficiency-trends profile, `stability_summary`, `correction_summary`, focused loop, closeout, router, and trend tests, final auto-closeout-checked 3-run trend proof with latest wall duration 4651 ms and estimated full receipt tokens 11383 | Add exact model token usage and exact correction counts when the runtime exposes them. |
