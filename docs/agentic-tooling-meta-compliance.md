...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, `agent:closeout:check --require-efficiency-metrics`, `agent:efficiency:trends --out`, durable `aipedia.loop-efficiency-trends.v1` receipts, `agent:closeout:check` loop-efficiency-trends validation, `stability_summary`, `correction_summary`, focused loop, closeout, and trend tests, final closeout-checked 3-run trend proof with latest wall duration 4608 ms and estimated full receipt tokens 10929 | Add exact model token usage and exact correction counts when the runtime exposes them. |
