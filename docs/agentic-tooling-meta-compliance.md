...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, exact model-token usage in loop receipts, workflow/run/orchestrator/subagent model-token breakdowns, exact model-token summaries in trend receipts, deterministic routing-evaluation receipts, correction-telemetry receipts, telemetry-fed routing evaluations, multi-scenario routing suite receipts, positive exact request and total-token validation, `agent:closeout:check --require-efficiency-metrics`, derived efficiency formula validation, artifact-ref count validation, slowest-command validation, trend run-row source validation, trend slowest-command source validation, trend summary validation, trend stability and correction source validation, trend exact-token source validation, token context breakdown validation, correction-telemetry total validation, routing score recomputation, routing-suite aggregate recomputation, `agent:efficiency:trends --out`, `agent:correction:telemetry --out`, `agent:routing:evaluate --out`, `agent:routing:suite --out`, durable `aipedia.loop-efficiency-trends.v1` receipts, durable `aipedia.correction-telemetry.v1` receipts, durable `aipedia.agent-routing-evaluation.v1` receipts, durable `aipedia.agent-routing-evaluation-suite.v1` receipts, `agent:closeout:check` loop-efficiency-trends, correction-telemetry, routing-evaluation, and routing-evaluation-suite validation, `agent:meta:closeout:auto` efficiency-trends, correction-telemetry, routing-evaluation, and routing-evaluation-suite profiles, `stability_summary`, `correction_summary`, focused loop, closeout, router, trend, correction telemetry, routing, and routing-suite tests, final auto-closeout-checked 3-run trend proof `.agent/evals/efficiency-trends-receipts/2026-06-30-slice-72-final-efficiency-trends.json`, correction telemetry proof `.agent/evals/correction-telemetry/2026-06-30-slice-73-correction-telemetry-receipt.json`, telemetry-fed routing proof `.agent/evals/routing-evaluations/2026-06-30-slice-73-routing-from-correction-telemetry-receipt.json`, and routing-suite proof `.agent/evals/routing-suites/2026-06-30-slice-74-routing-suite-receipt.json` showing conditional routing with 4,920 exact-token delta and 3,600 ms wall-time delta across recommended choices | Add runtime/reviewer adapters so events do not require manual fixtures, then validate task-class routing policy on real workloads before changing defaults. |
