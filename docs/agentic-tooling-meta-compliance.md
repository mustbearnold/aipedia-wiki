...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, `agent:closeout:check --require-efficiency-metrics`, derived efficiency formula validation, artifact-ref count validation, slowest-command validation, `agent:efficiency:trends --out`, durable `aipedia.loop-efficiency-trends.v1` receipts, `agent:closeout:check` loop-efficiency-trends validation, `agent:meta:closeout:auto` efficiency-trends profile, `stability_summary`, `correction_summary`, focused loop, closeout, router, and trend tests, final auto-closeout-checked 3-run trend proof `.agent/evals/efficiency-trends-receipts/2026-06-30-slice-60-final-efficiency-trends.json` with latest wall duration 4680 ms and estimated full receipt tokens 11437 | Add exact model token usage and exact correction counts when the runtime exposes them. |
