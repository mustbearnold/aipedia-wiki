...
| DAG contracts | Partial | `runner:agent-plan`, `agent:dag:check`, validated `aipedia.agent-task-dag.v1` node contract, focused DAG tests, `.agent/evals/agent-dag-contracts/2026-06-30-slice-33-cursor-agent-task-graph.json`, and architecture docs | Wire DAG validation into planner or closeout receipts so it cannot be skipped. |
