...
| Speed/token efficiency | Partial | six-worker workflows, timing receipts, loop `efficiency_metrics`, `agent:closeout:check --require-efficiency-metrics`, `agent:efficiency:trends --out`, durable `aipedia.loop-efficiency-trends.v1` receipts, `agent:closeout:check` loop-efficiency-trends validation, `agent:meta:closeout:auto` efficiency-trends profile, `stability_summary`, `correction_summary`, focused loop, closeout, router, and trend tests, final auto-closeout-checked 3-run trend proof with latest wall duration 4710 ms and estimated full receipt tokens 11462 | Add exact model token usage and exact correction counts when the runtime exposes them. |
